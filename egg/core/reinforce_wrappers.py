...
class RnnReceiverReinforceDeterministic(nn.Module):
    """
    ...
    def __init__(
        ...
        super(RnnReceiverReinforceDeterministic, self).__init__()
        self.agent = agent
        self.encoder = RnnEncoder(vocab_size, embed_dim, hidden_size, cell, num_layers)
        self.vocab_size = vocab_size
        self.reinforce = True

    def reinforce_mode(self):
        self.reinforce = True
    ...
    def forward(self, message, input=None, aux_input=None, lengths=None):
        ...
        if not self.reinforce:
            log_probs = torch.zeros(agent_output.size(0)).to(agent_output.device)
            entropy = log_probs
        elif self.reinforce:
            # agent output needs to be sampled. i.e. each object attribute (mini-vector) needs to be a distribution
            # and an element sampled from each distribution.
            # print('agent output shape: ', agent_output.shape)
            agent_output_size_1 = agent_output.size(1)
            agent_output = agent_output.view(
                agent_output.size(0),
                int(agent_output.size(1) / (self.vocab_size - 1)),
                self.vocab_size - 1
            )
            distr = Categorical(logits=F.log_softmax(agent_output, dim=-1))
            entropy = distr.entropy()
            sample = distr.sample() # 512 x 6 (good, these are labels per-attribute)
            log_probs = distr.log_prob(sample) # 512 x 6
            agent_output = agent_output.view(
                agent_output.size(0),
                agent_output_size_1
            )
...
<<<<<<< HEAD
        ...
        receiver_output, log_prob_r, entropy_r, _ = receiver(
        ...
        # print(660)
        # print('message size:', message.shape)
        # print('entropy_s size: ', entropy_s.shape)
