...
# from egg.zoo.imitation_learning.behavioural_cloning import train_bc
...
def ask_sender(n_attributes, n_values, dataset, device):
    attributes = []
    meanings = []
...
def context_independence(n_attributes, n_values, dataset, strings, device) -> float:
    ...
    attributes, _ = ask_sender(
        n_attributes, n_values, dataset, device
    )
    ...
    concept_set = set(
        [(att, val.item()) for object in attributes for att, val in enumerate(object)])  # concept = (att,val) tuple
...
class CompoEvaluator(core.Callback):
    def __init__(self, dataset, device, n_attributes, n_values, vocab_size, is_distributed, is_population=False,
                 epochs=[]):
